...
//      ("I$ blocked",                        () => icache_blocked),
  ...
  val spec_wakeup_issuop = Wire(Vec(exe_units.numIrfReaders, Bool()))
  for(w <- 0 until exe_units.numIrfReaders) {
    spec_wakeup_issuop(w) := iss_valids(w) && (iss_uops(w).iw_p1_poisoned || iss_uops(w).iw_p2_poisoned)
  }
  for(w <- 0 until exe_units.numIrfReaders) {
    spec_miss_issuop(w) := spec_wakeup_issuop(w) && io.lsu.ld_miss 
  }
  ...
  when (startCounter) {
    ...
    event_counters.io.event_signals(26) :=  PopCount(spec_wakeup_issuop.asUInt)   //spec wakeup total count
    event_counters.io.event_signals(27) :=  Mux(RegNext(io.lsu.spec_ld_wakeup_is_retry.reduce(_||_)),PopCount(spec_wakeup_issuop.asUInt) , 0.U) //spec wakeup from retry path count spec_miss_issuop
    event_counters.io.event_signals(28) :=  PopCount(spec_miss_issuop.asUInt)                   //wrong spec wakeup count
    event_counters.io.event_signals(29) :=  Mux(RegNext(io.lsu.spec_ld_wakeup_is_retry.reduce(_||_)), PopCount(spec_miss_issuop.asUInt), 0.U)  //wrong spec wakeup from retry path count
  }
