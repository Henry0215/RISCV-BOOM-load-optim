  ...
  with HasBoomFrontendParameters // TODO: Don't add this trait
{
  ...
  // Design: Store last memory access physical address (base_reg + last_imm)
  //         to enable fast physical address calculation without TLB
  //
  // Structure: Direct mapped (32 entries, indexed by logical register)
  //   For each entry, store:
  //     - last_imm: The immediate offset used in last access
  //     - last_paddr: Physical address of (base_reg_value + last_imm)
  //
  //   Address calculation: paddr = last_paddr + (curr_imm - last_imm)
  //
 val numLregs = 32  // Number of logical registers
  ...
  val cmap_last_imm = Reg(Vec(numLregs, UInt(12.W)))  // Last instruction's immediate offset
  val cmap_last_vaddr = Reg(Vec(numLregs, UInt(coreMaxAddrBits.W)))  // Virtual address: base + last_imm
  ...
  //-------------------------------------------------------------
  // Compute global CMAP invalidation signal (used for pipeline flush)
  // This signal will be propagated via brupdate.b1.cmap_flush to invalidate addresses in dispatch/issue
  ...
  // Propagate CMAP flush signal through brupdate to all pipeline stages
...
//      ("I$ blocked",                        () => icache_blocked),
  ...
  for(w <- 0 until coreWidth) {
    ...
    cmap_valid_not_same_page_cnt(w) := false.B
  }
  ...
  //   2. If yes, compute virtual address directly: vaddr = last_vaddr + (curr_imm - last_imm) + pending_offset
  ...
  // Track which Load instructions reset pending_offset for which register
  // This is used by ADDI to know whether to accumulate or start fresh
  // Set when Load's lrs1 (base register) has no valid CMAP entry (miss)
  val load_resets_pending_offset = Wire(Vec(coreWidth, Bool()))
  val load_resets_which_reg = Wire(Vec(coreWidth, UInt(log2Ceil(numLregs).W)))
  
  for (w <- 0 until coreWidth) {
    load_resets_pending_offset(w) := false.B
    load_resets_which_reg(w) := 0.U
  }
  ...
  for (w <- 0 until memWidth) {
    ...
    when (io.lsu.cmap_update_valid(w) && !cmap_global_invalidate) {
      ...
      val new_vaddr = io.lsu.cmap_update_vaddr(w)
      ...
      when (cmap_processing(lreg) && update_seq === cmap_processing_seq(lreg)) {
        // Update CMAP entry with new virtual address
        cmap_valid(lreg) := true.B
        cmap_processing(lreg) := false.B  // Clear processing bit
        cmap_last_imm(lreg) := io.lsu.cmap_update_imm(w)
        cmap_last_vaddr(lreg) := new_vaddr
        ...
        printf("[CMAP UPDATE] C=%d lreg=%d, new_vaddr=0x%x, imm=%d, pending_offset=%d, update_seq=%d, proc_seq=%d\n",
          debug_tsc_reg, lreg, new_vaddr, io.lsu.cmap_update_imm(w), cmap_pending_offset(lreg),
  ...
  for (w <- 0 until coreWidth) {
    ...
    // CMAP fast address calculation: Only for Load instructions
    // Store instructions (STA/STD/AMO) use normal pipeline for simplicity
    ...
    // x0 cannot be renamed and its value never changes
    val rs1_is_x0 = (rs1 === 0.U)
    
    // CRITICAL: Skip LR (Load Reserved) instructions - they have special memory ordering
    // requirements and should always go through normal TLB path
    val is_lr = dec_uops(w).mem_cmd === M_XLR

    // CRITICAL: Skip loads where rd == rs1 (e.g. "ld x10, 8(x10)")
    // Such loads overwrite the base register with the loaded value.
    // If CMAP caches the address computed from old rs1, the next load
    // using the *new* rs1 value would get a stale CMAP prediction.
    // Single-Register Invalidation fires on the same cycle as Load processing
    // for the same register, causing a Chisel last-writer-wins race that
    // prevents proper seq invalidation.
    val is_self_load = dec_uops(w).ldst_val && (dec_uops(w).ldst === rs1)
    
    // Check if an earlier Load in same cycle is setting processing for same rs1
    // This prevents two Loads from getting the same sequence number
    val earlier_load_sets_processing = if (w == 0) false.B else {
      (0 until w).map { i =>
        dec_valids(i) && dec_uops(i).uses_ldq && 
        dec_uops(i).lrs1 === rs1 && 
        dec_uops(i).lrs1 =/= 0.U &&  // x0 doesn't set processing
        dec_uops(i).cmap_will_update  // Earlier Load actually will set processing (precise check)
      }.reduce(_ || _)
    }
    
    when (is_load && dec_valids(w) && !rs1_is_x0 && !is_lr && !is_self_load) {
      // rs1 already defined above
      // For I-type instructions (Load), imm[11:0] = inst[31:20] = imm_packed[19:8]
      val imm = dec_uops(w).imm_packed(19, 8)  // Immediate offset (12-bit)
      
      // Step 0a: Check if any earlier instruction in same cycle writes to rs1
      // In superscalar pipeline, if an earlier instruction renames rs1, 
      // the CMAP entry for rs1 will be invalidated and should not be used
      // Exception: ADDI self-increment does NOT block (it accumulates pending_offset instead)
      // ADDI optimization DISABLED: all writes to rs1 block CMAP (including ADDI self-increment)
      val earlier_rename_checks = (0 until w).map { i =>
        val prev_uop = dec_uops(i)
        val prev_valid = dec_valids(i)
        prev_valid && prev_uop.ldst_val && prev_uop.ldst === rs1
      }
      val blocked_by_earlier_rename = if (w == 0) false.B else earlier_rename_checks.reduce(_ || _)
      
      // Step 0b: Check if current cycle has rename for rs1 (bypass check)
      // Also accumulate pending offset from same-cycle ADDI instructions
      // Use sequential Wires to avoid Chisel scope escape issues
      ...
      // ADDI optimization DISABLED: all writes to rs1 simply block CMAP
      for (i <- 0 until w) {
        val prev_uop = dec_uops(i)
        val prev_valid = dec_valids(i)
        val prev_writes_rs1 = prev_valid && prev_uop.ldst_val && prev_uop.ldst === rs1
        
        when (prev_writes_rs1) {
          // Any write to rs1 blocks CMAP
      ...
      // Final values - only use same-cycle blocking result
      // No need to check rename stage, all dependencies are resolved in decode
      ...
      // Step 0c: Combine all invalidation conditions
      val cmap_blocked = blocked_by_earlier_rename || 
                        blocked_by_current_rename || 
                        same_cycle_overflow ||
                        cmap_global_invalidate
      
      // DEBUG: Print blocking conditions
      ...
      // Step 1: Check if CMAP has valid entry for this register
      when (cmap_valid(rs1) && !cmap_blocked) {
        // Step 2: Compute virtual address directly using CMAP
        // With vaddr-based CMAP, no page boundary check is needed.
        // vaddr = last_vaddr + (curr_imm - last_imm) + pending_offset + same_cycle_offset
        // This is pure arithmetic on virtual addresses.
        val imm_signed = imm.asSInt.pad(15)  // Sign-extend 12-bit to 15-bit
        val last_imm_signed = cmap_last_imm(rs1).asSInt.pad(15)  // Sign-extend
        val offset_delta = imm_signed - last_imm_signed  // Safe from overflow in 15-bit
        val stored_pending = cmap_pending_offset(rs1).pad(15)  // Extend to 15-bit
        val total_offset_delta = offset_delta + stored_pending + same_cycle_pending_offset

        // Compute virtual address
        val base_vaddr_signed = cmap_last_vaddr(rs1).zext  // Zero-extend to (coreMaxAddrBits+1)-bit signed
        val offset_signed = total_offset_delta.pad(coreMaxAddrBits + 1)  // Sign-extend
        val vaddr_signed = base_vaddr_signed + offset_signed
        val vaddr = vaddr_signed(coreMaxAddrBits - 1, 0).asUInt  // Truncate back to coreMaxAddrBits unsigned
        
        dec_uops(w).cmap_addr_ready := true.B
        dec_uops(w).cmap_vaddr := vaddr
        dec_uops(w).cmap_will_update := false.B  // No need to update, entry is valid
        
        // CMAP Debug: Print prediction details at decode stage
        when (dec_fire(w)) {
          printf("[CMAP DECODE READY] C=%d slot=%d rob_idx=%d pc=0x%x vaddr=0x%x uses_ldq=%d iq_type=0x%x\n",
            debug_tsc_reg,
            w.U, dec_uops(w).rob_idx, dec_uops(w).debug_pc, vaddr,
            dec_uops(w).uses_ldq, dec_uops(w).iq_type)
        }
      } .otherwise {
        // CMAP entry not valid or blocked
        // Set processing bit if not already processing AND no earlier Load in same cycle
        // is setting processing for the same register (prevents sequence number collision)
        when (!cmap_blocked && !cmap_processing(rs1) && !earlier_load_sets_processing) {
          val new_seq = cmap_processing_seq(rs1) + 1.U
          dec_uops(w).cmap_will_update := true.B
          dec_uops(w).cmap_seq := new_seq  // Save sequence number to uop for later validation
          // CRITICAL: Only update CMAP registers when instruction actually fires
          when (dec_fire(w)) {
            cmap_processing(rs1) := true.B
            cmap_processing_seq(rs1) := new_seq  // Increment sequence number
            cmap_pending_offset(rs1) := 0.S  // Reset, will be overwritten by same-cycle ADDI if exists
            printf("[CMAP SET_PROCESSING] C=%d slot=%d rs1=%d new_seq=%d old_seq=%d pc=0x%x rob_idx=%d br_mask=0x%x\n",
              debug_tsc_reg, w.U, rs1, new_seq, cmap_processing_seq(rs1),
              dec_uops(w).debug_pc, dec_uops(w).rob_idx, dec_uops(w).br_mask)
          }
          // Track that this Load resets pending_offset for this register (combinational, for same-cycle ADDI)
          load_resets_pending_offset(w) := true.B
          load_resets_which_reg(w) := rs1
        }
      }
    }
  }
  ...
  //-------------------------------------------------------------
  // CMAP ADDI Optimization at Decode Stage - DISABLED for FPGA debugging
  //-------------------------------------------------------------
  // ADDI self-increment is now treated as a normal register write that
  // invalidates the CMAP entry. No offset accumulation is performed.
  // The addi_same_cycle_offset/overflow wires are kept but always zero
  // to avoid breaking any remaining references.
  ...
  for (w <- 0 until coreWidth+1) {
    for (i <- 0 until numLregs) {
      addi_same_cycle_offset(w)(i) := 0.S
      addi_same_cycle_overflow(w)(i) := false.B
    }
  }
  ...
  //-------------------------------------------------------------
  // MOVED HERE (after Load processing) to ensure correct Chisel last-writer-wins priority.
  // When a later-slot instruction (e.g., ADDI) invalidates a register that an earlier-slot
  // Load just set processing on, the invalidation must WIN (be the last writer).
  // If this were before Load processing, the Load's SET_PROCESSING would override the
  // invalidation, causing stale CMAP entries.
  //
  // When an instruction will write to a destination register (ldst_val),
  // invalidate its CMAP entry.
  ...
  for (w <- 0 until coreWidth) {
    when (dec_fire(w) && dec_uops(w).ldst_val && dec_uops(w).dst_rtype === RT_FIX) {
      val uop = dec_uops(w)
      val ldst = uop.ldst
      // ADDI optimization DISABLED for FPGA debugging:
      // All instructions that write to ldst now invalidate the CMAP entry,
      // including ADDI self-increment.
  ...
  // Clear CMAP physical addresses in dispatch stage if global flush occurs
  for (w <- 0 until coreWidth) {
    when (brupdate.b1.cmap_flush) {
