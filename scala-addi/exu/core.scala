  ...
  with HasBoomFrontendParameters // TODO: Don't add this trait
{
 ...
 val numLregs = 32  // Number of logical registers
  ...
  // SAB disabled in this configuration
...
//      ("I$ blocked",                        () => icache_blocked),
  ...
  //因投机唤醒而实际发射的指令（poisoned且被选中issue）
  ...
  when (startCounter) {
    ...
    event_counters.io.event_signals(24) :=  0.U  //SAB disabled
    event_counters.io.event_signals(26) :=  PopCount(spec_wakeup_issuop.asUInt)  //spec wakeup issued uop count (poisoned & issued)
    event_counters.io.event_signals(27) :=  0.U  //retry path disabled
    event_counters.io.event_signals(28) :=  PopCount(spec_miss_issuop.asUInt)  //actually mis-issued uop count due to wrong spec wakeup
    event_counters.io.event_signals(29) :=  0.U  //retry path disabled
  }
  ...
  for (w <- 0 until coreWidth) {
    ...
    when (is_cmap_candidate) {
      ...
      when (cmap_valid(rs1) && !cmap_blocked) {
        ...
        // STA: SAB disabled, no vaddr stored
